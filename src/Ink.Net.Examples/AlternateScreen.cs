// -----------------------------------------------------------------------
// AlternateScreen.cs — 1:1 port of examples/alternate-screen/alternate-screen.tsx
// A snake game rendered in the terminal's alternate screen buffer.
// -----------------------------------------------------------------------

using Ink.Net;
using Ink.Net.Builder;
using Ink.Net.Input;
using Ink.Net.SnakeGame;
using Ink.Net.Styles;
using Ink.Net.Terminal;

namespace Ink.Net.Examples;

/// <summary>
/// Snake game running on the alternate screen buffer.
/// <para>1:1 port of JS <c>examples/alternate-screen/alternate-screen.tsx</c>.</para>
/// </summary>
public static class AlternateScreenExample
{
    // Matches JS: headCharacter / bodyCharacter / foodCharacter / emptyCell
    private const string HeadChar  = "🦄";
    private const string BodyChar  = "✨";
    private const string FoodChar  = "🌈";
    private const string EmptyCell = "  ";
    private const int    TickMs    = 150;

    // Matches JS: rainbowColors (7 entries incl. orange)
    private static readonly string[] RainbowColors =
        ["red", "#FF7F00", "yellow", "green", "cyan", "blue", "magenta"];

    private static readonly string BorderH      = new string('─', SnakeGameEngine.BoardWidth * 2);
    private static readonly string BorderTop    = $"┌{BorderH}┐";
    private static readonly string BorderBottom = $"└{BorderH}┘";

    private static readonly Random Rng = new();

    // Matches JS: randomPosition(exclude)
    private static SnakePoint RandomPosition(IReadOnlyList<SnakePoint> exclude)
    {
        while (true)
        {
            var p = new SnakePoint(
                Rng.Next(SnakeGameEngine.BoardWidth),
                Rng.Next(SnakeGameEngine.BoardHeight));

            bool excluded = false;
            foreach (var seg in exclude)
            {
                if (seg.X == p.X && seg.Y == p.Y) { excluded = true; break; }
            }

            if (!excluded) return p;
        }
    }

    // Matches JS: createInitialState()
    private static SnakeGameState CreateInitialState()
    {
        var snake = new List<SnakePoint> { new(10, 7), new(9, 7), new(8, 7) };
        return new SnakeGameState
        {
            Snake    = snake,
            Food     = RandomPosition(snake),
            Score    = 0,
            GameOver = false,
            Won      = false,
            Frame    = 0,
        };
    }

    // Matches JS: buildBoard(snake, food)
    private static string BuildBoard(IReadOnlyList<SnakePoint> snake, SnakePoint food)
    {
        var headKey  = $"{snake[0].X},{snake[0].Y}";
        var snakeSet = new HashSet<string>();
        foreach (var seg in snake) snakeSet.Add($"{seg.X},{seg.Y}");

        var rows = new List<string> { BorderTop };
        for (int y = 0; y < SnakeGameEngine.BoardHeight; y++)
        {
            var row = "│";
            for (int x = 0; x < SnakeGameEngine.BoardWidth; x++)
            {
                var key = $"{x},{y}";
                if      (key == headKey)            row += HeadChar;
                else if (snakeSet.Contains(key))    row += BodyChar;
                else if (food.X == x && food.Y == y) row += FoodChar;
                else                                row += EmptyCell;
            }
            row += "│";
            rows.Add(row);
        }

        rows.Add(BorderBottom);
        return string.Join("\n", rows);
    }

    public static async Task RunAsync()
    {
        // Enable UTF-8 so emoji (🦄 ✨ 🌈) render correctly on Windows
        Console.OutputEncoding = System.Text.Encoding.UTF8;
        Console.InputEncoding  = System.Text.Encoding.UTF8;

        // Enter raw / VT-input mode (mirrors JS process.stdin.setRawMode(true)):
        // disables line-buffering so arrow keys arrive as ESC[A/B/C/D immediately.
        using var rawMode = new ConsoleRawMode();
        rawMode.Enter();

        var state     = CreateInitialState();
        var direction = SnakeDirection.Right;

        var app = InkApplication.Create(b => BuildTree(b, state), new InkApplicationOptions
        {
            AlternateScreen = true,
            ExitOnCtrlC     = false,
        });

        app.Input.Register((input, key) =>
        {
            if (input == "q")
            {
                app.Lifecycle.Exit();
                return;
            }

            if (state.GameOver && input == "r")
            {
                state     = CreateInitialState();
                direction = SnakeDirection.Right;
                app.Rerender(b => BuildTree(b, state));
                return;
            }

            if (state.GameOver) return;

            if      (key.UpArrow    && direction != SnakeDirection.Down)  direction = SnakeDirection.Up;
            else if (key.DownArrow  && direction != SnakeDirection.Up)    direction = SnakeDirection.Down;
            else if (key.LeftArrow  && direction != SnakeDirection.Right) direction = SnakeDirection.Left;
            else if (key.RightArrow && direction != SnakeDirection.Left)  direction = SnakeDirection.Right;
        });

        // Ctrl+C → graceful exit (mirrors JS useApp().exit() on SIGINT)
        Console.CancelKeyPress += (_, e) =>
        {
            e.Cancel = true;
            app.Lifecycle.Exit();
        };

        // stdin reading loop — read raw bytes from the OS stream so we get
        // exact VT sequences (ESC[A etc.) without StreamReader buffering layers.
        // A stateful UTF-8 Decoder handles any multi-byte char split across reads.
        _ = Task.Run(async () =>
        {
            try
            {
                var stdin  = Console.OpenStandardInput();
                var decoder = System.Text.Encoding.UTF8.GetDecoder();
                byte[] byteBuf = new byte[256];
                char[] charBuf = new char[256];
                while (!app.Lifecycle.HasExited)
                {
                    int byteCount = await stdin.ReadAsync(byteBuf, 0, byteBuf.Length);
                    if (byteCount <= 0) continue;
                    int charCount = decoder.GetChars(byteBuf, 0, byteCount, charBuf, 0);
                    if (charCount > 0) app.HandleInput(new string(charBuf, 0, charCount));
                }
            }
            catch { }
        });

        // Game tick loop
        _ = Task.Run(async () =>
        {
            while (!app.Lifecycle.HasExited)
            {
                await Task.Delay(TickMs);
                if (app.Lifecycle.HasExited) break;

                state = SnakeGameEngine.Tick(state, direction, RandomPosition);
                // Catch render exceptions so the tick loop never dies silently.
                try { app.Rerender(b => BuildTree(b, state)); }
                catch { }
            }
        });

        try { await app.WaitUntilExit(); } catch { }
        app.Dispose();
    }

    // Matches JS: SnakeGame() render return value
    private static TreeNode[] BuildTree(TreeBuilder b, SnakeGameState game)
    {
        var titleColor = RainbowColors[game.Frame % RainbowColors.Length];
        var board      = BuildBoard(game.Snake, game.Food);

        // Matches JS: const {columns} = useWindowSize()
        int columns = 80;
        try { columns = Console.WindowWidth; } catch { /* ignore */ }

        // Matches JS: boardWidthChars / marginLeft
        int boardWidthChars = SnakeGameEngine.BoardWidth * 2 + 2;
        int marginLeft      = Math.Max((columns - boardWidthChars) / 2, 0);

        // Matches JS conditional: game.gameOver ? ... : ...
        TreeNode statusRow = game.GameOver
            ? b.Box(
                new InkStyle { JustifyContent = JustifyContentMode.Center, MarginTop = 1 },
                [
                    // JS: <Text bold color="red">{won ? 'You Win!' : 'Game Over!'}{' '}</Text>
                    b.Text(
                        game.Won ? "You Win! " : "Game Over! ",
                        new InkStyle { Bold = true, Color = "red" }),
                    // JS: <Text dimColor>r: restart | q: quit</Text>
                    b.Text(
                        "r: restart | q: quit",
                        new InkStyle { DimColor = true }),
                ])
            : b.Box(
                new InkStyle { JustifyContent = JustifyContentMode.Center, MarginTop = 1 },
                [
                    // JS: <Text dimColor>Arrow keys: move | Eat {foodCharacter} to grow | q: quit</Text>
                    b.Text(
                        $"Arrow keys: move | Eat {FoodChar} to grow | q: quit",
                        new InkStyle { DimColor = true }),
                ]);

        // Matches JS: <Box flexDirection="column" paddingY={1}> ... </Box>
        return
        [
            b.Box(
                new InkStyle { FlexDirection = FlexDirectionMode.Column, PaddingY = 1 },
                [
                    // JS: <Box justifyContent="center"><Text bold color={titleColor}>🦄 Unicorn Snake 🦄</Text></Box>
                    b.Box(
                        new InkStyle { JustifyContent = JustifyContentMode.Center },
                        [b.Text("🦄 Unicorn Snake 🦄", new InkStyle { Bold = true, Color = titleColor })]),

                    // JS: <Box justifyContent="center" marginTop={1}><Text bold color="yellow">Score: ...</Text></Box>
                    b.Box(
                        new InkStyle { JustifyContent = JustifyContentMode.Center, MarginTop = 1 },
                        [b.Text($"Score: {game.Score}", new InkStyle { Bold = true, Color = "yellow" })]),

                    // JS: <Box marginLeft={marginLeft} marginTop={1}><Text>{board}</Text></Box>
                    b.Box(
                        new InkStyle { MarginLeft = marginLeft, MarginTop = 1 },
                        [b.Text(board)]),

                    statusRow,
                ]),
        ];
    }
}
